...
#include stan_functions/prior.stan
  ...
  real[,] integrate_ode_midpoint(real[] y0, real t0, real[] ts, 
    ...
    y[1] = y_tmp;
    for(i in 1:R_max){
      y_mid = y_tmp + 0.5*h*odefun(t_tmp, y_tmp, theta, x_r, x_i);
      y_tmp = y_tmp + h*odefun(t_tmp+0.5*h, y_mid, theta, x_r, x_i);
      t_tmp = t_tmp + h;
      y[i+1] = y_tmp;
    }
...
transformed parameters {
  ...
  // Solve ODE
  y = integrate_ode_midpoint(init, t0, ts, theta, step_size, INTERP_R, INTERP_A, x_r, x_i);
