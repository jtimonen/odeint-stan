...
y_out2 <- odeint(euler, y0, t0, t_out, theta, step_size = 0.2)
y_out3 <- odeint(rk4, y0, t0, t_out, theta, step_size = 0.2)
Y_out  <- list(y_out1, y_out2, y_out3)
plot_traj(t_out, Y_out, c(" RK45", "Euler (h=0.2)", "RK4 (h=0.2)"))
```

The output of RK45 and RK4 are indistinguishable in this case. Euler's method would need a much smaller step size to get even close in terms of global error.
...
fit  <- sampling(model, data, seed = stan_seed, refresh=0, iter=2000, chains=3)
print(fit)
```
...
Now we fit a model where ODEs are solved using our `rk4` solver with step size 0.2.
...
t_hat <- seq(0.05, 15, by=0.05)
THETA <- subsample_theta(fit, 100)
Y_hat <- solutions(data, rk4, THETA, t_hat)
plot_solutions(t_data, Y_data, t_hat, Y_hat, alpha=0.3)
```
