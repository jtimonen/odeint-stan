...
\end{cases}
$$
with $x_1(0) = 1$ and $x_2(0) = 1.5$. We create a Stan model.
```{r model1, cache=TRUE}
...
```{r sim, fig.width=6.5, fig.height=3}
# Simulation setup
set.seed(123)
...
## 3. Inference using RK45 from boost

### 3.1 Fitting the model
```{r fit1, fig.width=7.2, fig.height=5.5, cache=TRUE}
...
fit  <- sampling(model, data, seed = stan_seed, refresh=0)
print(fit)
```

We visualize the 4 chains
```{r diag1, fig.width=7.2, fig.height=5.5}
# Diagnose
plot_chains(fit, ncol=2)
```

We see that one chain was much slower than others and spent much of the warmup time on an area with very log probability (`lp__`).
```{r time1}
